...
Using this list (from the tidytext library) for sorting out the words we can redo the counting:
...
As before, we use unnest_tokens, but this time we specify that we want word pairs (bigrams). This time we filter out the preface (chapter 0)
